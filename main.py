...
##############################################################
    for epoch in range(args.epochs):
        local_losses = []#空列表用于本地模型的损失值
        local_weights = []
        f_locals = []
        args.g_epoch = epoch#记录当前是第几个epoch
        #实现学习率衰减（learning rate decay）的功能
        if (epoch + 1) in args.schedule:
            print("Learning Rate Decay Epoch {}".format(epoch + 1))
            print("{} => {}".format(args.lr, args.lr * args.lr_decay))#打印了学习率衰减前后的学习率值
        ...
        m = max(int(args.frac * args.num_users), 1)#计算了参与训练的用户数量
        idxs_users = np.random.choice(range(args.num_users), m, replace=False)#随机选择m个用户索引。参数 replace=False 确保所选用户不重复
        
        #本地更新
        for client_num, idx in enumerate(idxs_users):#遍历每个用户索引idx
        ...
        print('Round {:3d}'.format(epoch))#打印当前轮次（epoch）的序号，使用了字符串格式化来显示轮次数
        print(' - '.join([f'{k}: {v:.6f}' for k, v in results.items()]))# 这行代码打印每轮训练/测试的结果，将结果格式化为字符串，包括每个结果的名称和具体数值
